...
// GET /api/presensi — List presensi milik user yg login
export async function GET(req: NextRequest) {
  try {
    ...
    // If admin, can see all presensi; if mentor, only own
...
// POST /api/presensi — Create presensi baru
export async function POST(req: NextRequest) {
  try {
    ...
    const { tanggal, jamMasuk, durasiMenit, catatan } = body;
    ...
    if (!tanggal || !jamMasuk || !durasiMenit) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "Tanggal, jam masuk, dan durasi wajib diisi" },
        },
        { status: 422 }
      );
    }
    ...
    if (![60, 90, 120].includes(durasiMenit)) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "Durasi harus 60, 90, atau 120 menit" },
        },
        { status: 422 }
      );
    }
